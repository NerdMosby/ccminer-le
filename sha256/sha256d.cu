...
extern "C" void sha256d_hash_le(void *output, const void *input)
{
	unsigned char _ALIGN(64) hash[64];
	SHA256_CTX sha256;

	SHA256_Init(&sha256);
	SHA256_Update(&sha256, (unsigned char *)input, 112);
	SHA256_Final(hash, &sha256);
...
extern void sha256d_setBlock_112(uint32_t *pdata, uint32_t *ptarget);
extern void sha256d_hash_80(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *resNonces);
extern void sha256d_hash_112(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *resNonces);
...
extern "C" int scanhash_sha256d_le(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	sha256d_setBlock_112(endiandata, ptarget);

	do {
		// Hash with CUDA
		*hashes_done = pdata[27] - first_nonce + throughput;

		sha256d_hash_112(thr_id, throughput, pdata[27], work->nonces);
		//sha256d_hash_80(thr_id, throughput, pdata[27], work->nonces);
		if (work->nonces[0] != UINT32_MAX)
		{
			uint32_t _ALIGN(64) vhash[8];

			endiandata[27] = swab32(work->nonces[0]);
			...
			if (vhash[7] <= ptarget[7] && fulltest(vhash, ptarget)) {
				...
				if (work->nonces[1] != UINT32_MAX) {
					...
					pdata[27] = max(work->nonces[0], work->nonces[1]) + 1;
				}
				else {
		...
		if ((uint64_t)throughput + pdata[27] >= max_nonce) {
			pdata[27] = max_nonce;
			break;
		}
	...
	*hashes_done = pdata[27] - first_nonce;
