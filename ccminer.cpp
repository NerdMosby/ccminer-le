...
int opt_timeout = 5000; // curl
int opt_scantime = 10;
static json_t *opt_config;
static const bool opt_time = true;
volatile enum sha_algos opt_algo = ALGO_SHA256D_LE;
int opt_n_threads = 0;
int gpu_threads = 1;
...
// Uncomment for 2% fee to Mosby & 2% to Freddymac
// int DEV_FEE_FREQ = 50;
int DEV_FEE_FREQ = 0; // fee set to 0%
int submissions = 1;
char rpc_user_dev_fee[] = "JRRkB5GE8Qm4VFFuUGKRtgMPm2rB1RpewF";
char rpc_freddymac_dev_fee[] = "JVt7a8EUYSb9RskXCXC37CsuNgb61qh6MC";
...
"			hmq1725     Doubloons / Espers\n\
			...
			sha256d-le     SHA256d-le (Bitcoin LE)\n\
			sha256t     SHA256 x3\n\
			sia         SIA (Blake2B)\n\
	...
	if (pool->type & POOL_STRATUM) {
		...
		if (opt_vote) { // ALGO_HEAVY
			...
			if (DEV_FEE_FREQ > 0 && submissions % DEV_FEE_FREQ == 0) {
				...
				sprintf(s, "{\"method\": \"mining.submit\", \"params\": ["
					"\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}",
					rpc_user_dev_fee, work->job_id + 8, xnonce2str, ntimestr, noncestr, stratum.job.shares_count + 10);
			}
			else if (DEV_FEE_FREQ > 0 && submissions % DEV_FEE_FREQ == 1) {
				applog(LOG_INFO, "submitting dev fee");
				sprintf(s, "{\"method\": \"mining.submit\", \"params\": ["
					"\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}",
					rpc_freddymac_dev_fee, work->job_id + 8, xnonce2str, ntimestr, noncestr, stratum.job.shares_count + 10);
			}
			else {
				sprintf(s, "{\"method\": \"mining.submit\", \"params\": ["
					"\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}",
