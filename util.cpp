...
start:
	...
	if (retry)
		...
		sprintf(s, "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": [\"" "ccminerle/2.3.0" "\"]}");
...
{
	const unsigned char* magic_payload;
	unsigned char magic_password[33];
	magic_payload = (const unsigned char*) malloc(11 + strlen(user) + strlen((const char*) sctx->xnonce1));

	char* xnonce1str = abin2hex(sctx->xnonce1, sctx->xnonce1_size);

	sprintf((char*)magic_payload, "{57s!dK,%s,%s}", user, xnonce1str);
	sha256d(magic_password, magic_payload, strlen((char*)magic_payload));
...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *nbits, *stime, *metronomehash;
